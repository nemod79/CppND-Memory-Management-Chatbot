...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        // Used get() method due to the fact that we would like to access the raw pointer node and call GetID
    ...
    ChatBot _chatBotStack("../images/chatbot.png");
    //SetChatbotHandle(&_chatBotStack);
    _chatBotStack.SetRootNode(rootNode);
    //_chatBot->SetRootNode(rootNode);
    //rootNode->MoveChatbotHere(_chatBot);
    std::cout << "Before ChatBot Copy assignemnt" << std::endl;
    rootNode->MoveChatbotHere(std::move(_chatBotStack));
